...
def sdes(text: bitarray, key: bitarray, mode) -> bitarray:
    ...
    initial_permutation = bitarray()

    for i in IP:
        initial_permutation.append(text[i])
    ...
    if mode == 1:
        ...
        round2 = round(switch[4:8], key2)
        round2 ^= switch[0:4]

        encrypt = round2 + switch[4:8]
        ...
        round2 = round(switch[4:8], key1)
        round2 ^= switch[0:4]

        decrypt = round2 + switch[4:8]
